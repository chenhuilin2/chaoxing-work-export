...
function firstBoolean(source: Record<string, unknown>, keys: readonly string[], fallback: boolean): boolean {
...
function exportPreferencesValue(value: unknown): ExportPreferences {
  ...
  return {
    ...
    includeAnalysis: firstBoolean(
      source,
      ['includeAnalysis', 'withAnalysis', 'appendAnalysis'],
      DEFAULT_SETTINGS.exportPreferences.includeAnalysis,
    ),
...
function settingsValue(value: unknown): UserSettings {
  ...
  return {
    ...
    enableDrag: booleanValue(
      source.enableDrag ?? source.draggable,
...
export class SettingsRepository {
  ...
  load(): UserSettings {
    ...
    if (current !== null) return settingsValue(current);
