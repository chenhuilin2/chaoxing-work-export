      ...
      <section class="cwe-panel" data-open="false" role="dialog" aria-label="${APP_NAME}">
        <header class="cwe-header">
          <div class="cwe-header-main">
            ...
            <div class="cwe-subtitle">TypeScript 重构版 · v${APP_VERSION}</div>
          </div>
...
function createButton(text: string, action: string, danger = false): HTMLButtonElement {
...
export class PanelView {
  ...
  renderHistory(entries: readonly HistoryEntry[]): void {
    ...
    for (const entry of entries) {
      ...
      meta.textContent = `${entry.result.statistics.total} 题 · ${entry.options.format.toUpperCase()} · ${new Date(
        entry.createdAt,
      ).toLocaleString()}`;
      ...
      actions.append(
        createButton('恢复', 'restore'),
        createButton('重新下载', 'download'),
        createButton('删除', 'delete', true),
      );
  ...
  private bindEvents(): void {
    ...
    this.element<HTMLElement>('[data-history-list]').addEventListener('click', (event) => {
      const target = event.target instanceof Element ? event.target.closest<HTMLButtonElement>('[data-history-action]') : null;
      ...
      if (action === 'restore') this.callbacks.onHistoryRestore(id);
      else if (action === 'download') this.callbacks.onHistoryDownload(id);
